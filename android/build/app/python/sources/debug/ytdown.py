...
def _guess_artist_from_title(title):
    ...
    for separator in [" - ", " – ", " — ", " | "]:
        if separator in normalized:
            ...
            if candidate and not _is_unknown_label(candidate):
                return candidate
...
def _write_mp3_id3_tags(filepath, title, artist, album, thumbnail_url=None):
    ...
    try:
        ...
        # Verificação de escrita no arquivo físico.
        verify = ID3(filepath)
        verified_title = ""
        verified_artist = ""
        verified_album = ""

        if verify.get("TIT2") and verify.get("TIT2").text:
            verified_title = _normalize_tag_value(verify.get("TIT2").text)
        if verify.get("TPE1") and verify.get("TPE1").text:
            verified_artist = _normalize_tag_value(verify.get("TPE1").text)
        if verify.get("TALB") and verify.get("TALB").text:
            verified_album = _normalize_tag_value(verify.get("TALB").text)

        return (
            verified_title == _normalize_tag_value(title)
            and verified_artist == _normalize_tag_value(artist)
            and verified_album == _normalize_tag_value(album)
        )
    except Exception as e:
        print(f"❌ Falha ao gravar ID3 direto no MP3: {str(e)}")
        import traceback
...
def _write_mp4_m4a_tags(filepath, title, artist, album, thumbnail_url=None):
    ...
    try:
        ...
        audio["----:com.apple.iTunes:ALBUM"] = [
            MP4FreeForm(str(album).encode("utf-8"))
        ]
        audio["----:com.apple.iTunes:TITLE"] = [
            MP4FreeForm(str(title).encode("utf-8"))
        ]
        ...
        verify = MP4(filepath)
        verify_tags = verify.tags or {}

        title_ok = (
            _normalize_tag_value(verify_tags.get("\xa9nam"))
            == _normalize_tag_value(title)
        )
        artist_ok = (
            _normalize_tag_value(verify_tags.get("\xa9ART"))
            == _normalize_tag_value(artist)
        )
        album_artist_ok = (
            _normalize_tag_value(verify_tags.get("aART"))
            == _normalize_tag_value(artist)
        )
        album_ok = (
            _normalize_tag_value(verify_tags.get("\xa9alb"))
            == _normalize_tag_value(album)
        )

        if not (title_ok and artist_ok and album_artist_ok and album_ok):
            print(
                "⚠️ Verificação MP4/M4A falhou",
                {
                    "title_ok": title_ok,
                    "artist_ok": artist_ok,
                    "album_artist_ok": album_artist_ok,
                    "album_ok": album_ok,
                    "expected": {
                        "title": str(title),
                        "artist": str(artist),
                    ...
                    "actual": {
                        "title": verify_tags.get("\xa9nam"),
                        "artist": verify_tags.get("\xa9ART"),
                        "album_artist": verify_tags.get("aART"),
                        "album": verify_tags.get("\xa9alb"),
                    },
                },
            )

        return title_ok and artist_ok and album_artist_ok and album_ok
    except Exception as e:
